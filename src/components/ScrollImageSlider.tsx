...
const ScrollImageSlider = () => {
  ...
  // Pre-compute transforms for images
  const imageOpacities = [
    useTransform(scrollYProgress, [0, 0.33], [1, 0], { clamp: false }),
    useTransform(scrollYProgress, [0, 0.33, 0.66], [0, 1, 0], { clamp: false }),
    useTransform(scrollYProgress, [0.33, 0.66], [0, 1], { clamp: false })
  ];

  // Pre-compute transforms for quotes
  const quoteTransforms = quotes.map((_, i) => ({
    yPosition: useTransform(activeIndex, [i - 1, i, i + 1], [1000, 0, -1000], { clamp: false }),
    opacity: useTransform(activeIndex, [i - 0.5, i, i + 0.5], [0, 1, 0], { clamp: false })
  }));

  // Scroll indicator opacity
  const scrollIndicatorOpacity = useTransform(scrollYProgress, [0, 0.1], [1, 0], { clamp: false });
